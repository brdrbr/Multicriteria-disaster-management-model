...
directory = './data/sample_disaster_data.xlsx'
T, K, N, nS, nD, nN, nT, nK, nE, B, Sikt, djkt, Cijkt, uijt, blocked, bt, aij, edge_dict = data_reader.data_read(directory)
...
"""
        ...
        Target_Model = model_constraints(Target_Model, nD, nT, nK, nN, Sikt, djkt, nS, uijt, edge_dict, aij, bt,
                                         alpha=0.3)
        Target_Model.obj = Objective(expr=((Target_Model.obj_mincost * 0.001) + Target_Model.obj_unsatisfied), sense=1)
        Model_solution = opt.solve(Target_Model)
...
for i in range(0, 2):
    ...
    if target_problem == 'min_cost':
        ...
        Model_solution = opt.solve(Target_Model)
        scaled_terminal_writer(Target_Model, min1,  min4, scaling_factor_mincost, scaling_factor_unsatisfied, nT,
        nK, nS, djkt, problem)
